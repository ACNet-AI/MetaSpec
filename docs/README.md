# MetaSpec Documentation

> 📚 Documentation Index

---

## 🚀 Quick Start

- **[quickstart.md](./quickstart.md)** - 5-minute quick start guide

---

## 📐 Architecture & Specifications

- **[architecture.md](./architecture.md)** - Three-layer template architecture
- **[slash-cmd-protocol.md](./slash-cmd-protocol.md)** - Slash command dual-layer architecture
- **[spec-driven.md](./spec-driven.md)** - Specification-Driven X (SD-X) methodology
- **[community-registry.md](./community-registry.md)** - Community speckit registry

---

## 🤝 Contributing

- **[contributing.md](./contributing.md)** - Contribution guidelines and development standards

---

## 📚 Other Resources

### **AI Agent Guide**
- [AGENTS.md](../AGENTS.md) - Complete AI workflow guide

### **Examples**
- [examples/](../examples/) - Example collection (coming soon)

---

## 📞 Get Help

- **Main Project**: [README.md](../README.md)
- **Report Issues**: GitHub Issues

---

**Version**: v0.1.0 | **Last Updated**: 2025-10-31
